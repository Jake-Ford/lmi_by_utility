...
temp_util <- hifld_df %>%
  filter(ID %in% c(3249, 13511, 16183, 14154)) %>%
  mutate(new_name = case_when(
    ID == 3249 ~ "Central Hudson",
    ID == 13511 ~ "NYSEG",
    ID == 16183 ~ "Rochester Gas & Electric",
    ID == 14154 ~ "Orange & Rockland"
  ))
...
nat_grid_xy <- st_transform(nat_grid_xy, st_crs(get_tracts))
...
Solstice operates in the following three utility zones in New York:
...
map <- leaflet() %>%
  ...
  addLayersControl(
    overlayGroups=c( "80% AMI", "Utility Zones"),  # Update the order for consistency
    options = layersControlOptions(collapsed = FALSE)
  ) %>%
    addPolygons(data = nat_grid_xy, 
              fillColor = "blue", 
   ...
   addLegend(
    ...
    addLayersControl(
    overlayGroups=c("80% AMI", "Utility Zones", "NatGrid Zone E"),  # Update the order for consistency
    options = layersControlOptions(collapsed = FALSE)
  )
